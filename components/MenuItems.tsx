...
export default function MobileMenuItems({ onItemClick }: MobileMenuItemsProps) {
    ...
    return (
        <div className="fixed bg-background h-full w-full z-50 overflow-hidden">
                ...
                {item.map((item) => (
                    <li key={item.title} onClick={onItemClick}>
                        <Link
                            href={item.link}
                            className={clsx(
                                'flex h-10 items-center my-2 gap-2 rounded-md px-2 text-sm hover:bg-muted',
                                pathname.includes(item.link) && 'bg-muted font-bold',
                            )}
                        >
